...
For a friendlier way to edit, validate, and run a golden set than a
terminal, there's an optional local web GUI:

```bash
pip install "idem-check[gui]"
idem gui
```

This opens a browser tab with a YAML editor on one side and live
validate/run results on the other — pass/fail per question, the exact
`detail` string for every check, and download buttons for `report.md`
and `audit_log.jsonl`. It's a thin front end over the same `idem.config`,
`idem.runner`, and `idem.target` code the CLI uses, so a report generated
in the GUI is identical to one generated by `idem run` — the YAML file
stays the real, auditable source of truth either way.

This is a local, single-user tool with no authentication built in —
don't expose it on a public network without adding your own auth in
front of it. API keys entered in the GUI are kept only in that process's
